...
public class GoalDaoImpl extends AbstractDao<Goal> implements GoalDao {
    ...
    public boolean update(Goal goal) throws DaoException {
        return executeUpdate(SqlConstant.UPDATE_GOAL.getName(), goal.getName(), goal.getDescription(), goal.getBeginDate(), goal.getEndDate(), goal.getUserId(), goal.getTypeId(), goal.getId());
    }

    @Override
    public boolean delete(Goal goal) throws DaoException {
        return executeUpdate(VariableConstant.DELETE_GOAL.getName(), goal.getId());
    }
    ...
    public List<Goal> findGoalByDate(Date beginDate, Date endDate) {
        return executeQuery(SqlConstant.GET_GOALS_BY_DATE.getName(), beginDate, endDate);
    }

    @Override
    public List<Goal> findGoalByName(String name) {
        return executeQuery(SqlConstant.GET_GOALS_BY_NAME.getName(), name);
    }
}
